...
class DashboardFrame(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        if logo_img:
            ctk.CTkLabel(self.head, image=logo_img, text="").pack(side="left", padx=(15, 5), pady=10)
        
        title_frame = ctk.CTkFrame(self.head, fg_color="transparent")
        title_frame.pack(side="left", padx=5, pady=10)
        
        self.lbl_title = ctk.CTkLabel(title_frame, text="Rapport", font=("Segoe UI", 24, "bold"), text_color="white")
        self.lbl_title.pack(anchor="w")
        
        self.lbl_subtitle = ctk.CTkLabel(title_frame, text="Wachten op data...", font=("Segoe UI", 13), text_color="#94a3b8")
        self.lbl_subtitle.pack(anchor="w")
        
        self.btn_logout = ctk.CTkButton(self.head, text="🚪 Uitloggen", font=("Segoe UI", 13, "bold"), width=120, fg_color="transparent", border_width=1, border_color="#e2e8f0", command=self.logout)
        self.btn_logout.pack(side="right", padx=20, pady=20)
        
        self.btn_new_scan = ctk.CTkButton(self.head, text="🔄 Nieuwe Scan", font=("Segoe UI", 13, "bold"), width=120, fg_color=COLOR_ACCENT, text_color="black", hover_color="#b0841a", command=self.new_scan)
        self.btn_new_scan.pack(side="right", padx=10, pady=20)
        ...
        self.btn_total = ctk.CTkButton(self.nav_frame, text="Totaal Overzicht", font=("Segoe UI", 14, "bold"), command=lambda: self.switch_tab(self.btn_total, self.view_total))
        self.btn_total.pack(side="left", expand=True, padx=5)

        self.btn_sp = ctk.CTkButton(self.nav_frame, text="SharePoint", font=("Segoe UI", 14, "bold"), command=lambda: self.switch_tab(self.btn_sp, self.view_sp))
        self.btn_sp.pack(side="left", expand=True, padx=5)

        self.btn_local = ctk.CTkButton(self.nav_frame, text="Lokale Schijf", font=("Segoe UI", 14, "bold"), command=lambda: self.switch_tab(self.btn_local, self.view_local))
        self.btn_local.pack(side="left", expand=True, padx=5)
        
        self.btn_files = ctk.CTkButton(self.nav_frame, text="🔍 Bestandenlijst", font=("Segoe UI", 14, "bold"), command=lambda: self.switch_tab(self.btn_files, self.view_files))
        self.btn_files.pack(side="left", expand=True, padx=5)
        
        self.filter_var = ctk.StringVar(value="Alle Afdelingen")
        self.opt_filter = ctk.CTkOptionMenu(self.nav_frame, values=["Alle Afdelingen", "HR", "Finance", "IT", "Directie"], variable=self.filter_var, font=("Segoe UI", 14))
        ...
        self.btn_export = ctk.CTkButton(self.export_frame, text="📥 Exporteer Data (CSV)", font=("Segoe UI Black", 14), text_color="white", height=50, command=self.export_to_csv)
    ...
    def load_data(self, data, afdeling):
        ...
        self.lbl_subtitle.configure(text=f"{len(base_results)} bestanden geanalyseerd  |  {critical_count} kritieke problemen")
        ...
        if is_admin:
            ...
            ctk.CTkLabel(admin_view, text="🛡️ Administrator Scan Voltooid", font=("Segoe UI Black", 30), text_color=COLOR_ACCENT).pack(pady=(20, 10))
            ...
            ctk.CTkLabel(admin_view, text=info_text, font=("Segoe UI", 16), text_color="#e2e8f0").pack(pady=20)
            
            btn_pb = ctk.CTkButton(admin_view, text="📊 Download PowerBI Dataset (.csv)", font=("Segoe UI Black", 18), fg_color=COLOR_ACCENT, hover_color="#1d4ed8", height=60, width=450, command=self.export_to_csv)
            btn_pb.pack(pady=30)
            
            btn_show_list = ctk.CTkButton(admin_view, text="👀 Bekijk Bestandenlijst (In-App)", font=("Segoe UI", 14), fg_color="transparent", border_width=1, border_color="#334155", hover_color="#1e293b", height=40, width=300, command=self.show_admin_file_list)
            ...
            risico_bestanden = [res["Naam"] for res in self.results_filtered if "🚨 KRITIEK" in res.get("Reden", "")]
            if risico_bestanden: self._build_alert(self.view_total, risico_bestanden)
            ...
            if len(base_results) > 50:
                ctk.CTkLabel(self.view_total, text="Let op: Als 'User' ziet u hier uw top 50 prioriteitsbestanden.", text_color=COLOR_WARN, font=("Segoe UI", 13, "bold")).pack(pady=10)
    ...
    def show_admin_file_list(self):
        ...
        lbl_warn = ctk.CTkLabel(self.view_files, text="⚠️ Beheerdersmodus: Alle bestanden zijn ingeladen. Navigeren kan vertraagd zijn.", text_color=COLOR_WARN, font=("Segoe UI", 13, "bold"))
    ...
    def _build_alert(self, parent_frame, risico_bestanden):
        alert_frame = ctk.CTkFrame(parent_frame, fg_color="#7f1d1d", corner_radius=10, border_width=2, border_color="#ef4444")
        ...
        if len(risico_bestanden) > 5: files_text += f"\n... en nog {len(risico_bestanden) - 5} andere(n)."
    ...
    def _calc_average(self, domain_dict):
        ...
        for scores in domain_dict.values():
            ...
            numeric_scores = []
            for s in scores:
                if isinstance(s, str): s = s.replace('%', '').strip()
                try: numeric_scores.append(float(s))
    ...
    def _build_tab_content(self, parent_frame, domain_dict):
        ...
        # 2. Header Frame voor de Scores
        header_frame = ctk.CTkFrame(parent_frame, fg_color="transparent")
        header_frame.pack(fill="x", pady=10)

        def _build_score_box(parent, title, avg):
            box = ctk.CTkFrame(parent, fg_color=COLOR_BG_LIGHT, corner_radius=6, border_width=1, border_color="#334155")
            box.pack(side="left", fill="both", expand=True, padx=10)
            ctk.CTkLabel(box, text=title, font=("Segoe UI", 16, "bold"), text_color="#e2e8f0").pack(pady=(20, 5))
            if avg == -1:
                ctk.CTkLabel(box, text="N/A", font=("Segoe UI Black", 42), text_color="#475569").pack(pady=(0, 20))
            else:
                kleur = COLOR_PASS if avg >= 75 else (COLOR_WARN if avg >= 50 else COLOR_FAIL)
                ctk.CTkLabel(box, text=f"{avg:.1f}%", font=("Segoe UI Black", 42), text_color=kleur).pack(pady=(0, 20))

        if active_comp or central_comp_keys: _build_score_box(header_frame, "🛡️ Compliance Score", comp_avg)
        if active_qual: _build_score_box(header_frame, "✨ Data Quality Score", qual_avg)

        # 3. Body voor de regels
        ...
        def _build_rules_list(title, rules_dict):
            ...
            # Check of het dict überhaupt data heeft
            has_data = any(len(v) > 0 for v in rules_dict.values())
            if not has_data: return

            ctk.CTkLabel(body, text=title, font=("Segoe UI", 20, "bold"), text_color=COLOR_ACCENT).pack(anchor="w", padx=10, pady=(20, 10))
            ...
            for mod, scores in rules_dict.items():
                ...
                # STOPLICHT LOGICA
                if mod_avg >= 75: light = "🟢"
                elif mod_avg >= 50: light = "🟡"
                else: light = "🔴"
                
                row = ctk.CTkFrame(body, fg_color=COLOR_BG_LIGHT, corner_radius=6, border_width=1, border_color="#334155")
                row.pack(fill="x", padx=10, pady=5)
                
                mod_title = f"{light}  {mod}  •  {mod_avg:.1f}%"
                ctk.CTkLabel(row, text=mod_title, font=("Segoe UI", 15, "bold"), text_color="white").pack(anchor="w", padx=20, pady=(12, 5))
                ...
                ctk.CTkLabel(row, text=text_reasons, font=("Consolas", 13), text_color=c, justify="left").pack(anchor="w", padx=40, pady=(0, 15))

        if active_comp or central_comp_keys: _build_rules_list("Regels: Compliance", comp_dict)
        if active_qual: _build_rules_list("Regels: Data Quality", qual_dict)
    ...
    def _build_files_tab(self):
        ...
        self.search_entry = ctk.CTkEntry(search_frame, textvariable=self.search_var, placeholder_text="Zoek op bestandsnaam...", font=("Segoe UI", 14), width=300)
        self.search_entry.pack(side="left", padx=(0, 10))
        
        # Activering van de zoekopdracht bij drukken op Enter of Knop
        btn_search = ctk.CTkButton(search_frame, text="Zoeken", command=self.apply_search)
        ...
        ctk.CTkLabel(header, text="Bestand & Reden", font=("Segoe UI", 14, "bold")).pack(side="left", padx=10, fill="x", expand=True)
        ctk.CTkLabel(header, text="Locatie", font=("Segoe UI", 14, "bold")).pack(side="right", padx=10)
        
        self.file_list_container = ctk.CTkScrollableFrame(self.view_files, fg_color=COLOR_BG_LIGHT)
        ...
        self.btn_prev = ctk.CTkButton(page_frame, text="< Vorige", command=self.prev_page, width=100)
        self.btn_prev.pack(side="left", padx=10)
        
        self.lbl_page = ctk.CTkLabel(page_frame, text="Pagina 1 van 1", font=("Segoe UI", 14))
        self.lbl_page.pack(side="left", expand=True)
        
        self.btn_next = ctk.CTkButton(page_frame, text="Volgende >", command=self.next_page, width=100)
    ...
    def apply_search(self):
        ...
        if not query:
            ...
            self.filtered_search_results = [r for r in self.results_filtered if query in r.get("Naam", "").lower() or query in r.get("Reden", "").lower()]
    ...
    def render_file_page(self):
        ...
        for item in page_items:
            ...
            if "0%" in item.get("Score_Totaal", ""): score_color = COLOR_FAIL
            
            title_text = f"{item.get('Naam', '')}  [{item.get('Score_Totaal', '')}]"
            ctk.CTkLabel(row, text=title_text, font=("Segoe UI", 14, "bold"), text_color=score_color).pack(anchor="w", padx=10, pady=(5,5))
            
            redenen = item.get('Reden', '').split(" | ")
            for r in redenen:
                if r.strip():
                    if "KRITIEK" in r:
                        r_color = "#fca5a5"
                        ...
                        r_color = "#e2e8f0"
                        icon = "⚠️"
                    ctk.CTkLabel(row, text=f"{icon} {r.strip()}", font=("Consolas", 13), text_color=r_color, justify="left", wraplength=700).pack(anchor="w", padx=25, pady=(0, 2))
                    
            # Padding extraatje onderaan de box
            ctk.CTkLabel(row, text="", height=2).pack()
            
            pad = item.get('Pad', '')
            parts = pad.replace('\\', '/').split('/')
            kort_pad = ".../" + "/".join(parts[-3:]) if len(parts) > 3 else pad
            path_lbl = ctk.CTkLabel(row, text=kort_pad, font=("Segoe UI", 11, "italic"), text_color="#888888")
            path_lbl.place(relx=1.0, rely=0.05, anchor="ne", x=-10, y=10)
    ...
    def export_to_csv(self):
        ...
        for row in self.full_dataset:
            ...
            is_warn = ("⚠️" in reden_str) or ("Locatie:" in reden_str) or ("Duplicatie:" in reden_str) or ("Waarschuwing" in reden_str)
            ...
            aantal_overtredingen = 0 if not reden_str or "Compliant" in reden_str else len(reden_str.split(" | "))
            
            clean_reden = reden_str.replace('\n', ' ').strip()
        ...
        try:
            ...
            messagebox.showinfo("Export Succesvol", f"PowerBI Dataset is succesvol opgeslagen in:\n{filepath}\n\nDeze is ingericht met vaste filters en direct klaar voor inlezen!")
        except PermissionError: messagebox.showerror("Export Fout", "Kan het bestand niet overschrijven. Is het geopend in Excel of PowerBI?")
        except Exception as e: messagebox.showerror("Export Fout", f"Er is een fout opgetreden:\n{e}")
